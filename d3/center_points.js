...
d3.csv("data/center_stats.csv").then(function(data) {
    ...
    y.domain(d3.extent(data, function(d) { return d.PTS; })).nice();
    ...
    var tipMouseover = function(event, d) {
        ...
        tooltip.html(tipcontent)
            .style("left",  (d3.pointer(event)[0] + 25) + "px") // or d3.pointer(event)[0]
            .style("top", (d3.pointer(event)[1] - 5) + "px") // or d3.pointer(event)[1]
        };
    // tooltip mouseout
    ...
    if (jokic) {
        svg.append("text")
           ...
           .text("Jokic is one of the top scoring centers")
           .attr("font-size", "12px")
